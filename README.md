...
- При выполнении функции `main()`для этого экземпляра вызвается метод `show_training_info()`;
результат выполнения метода - объект класса `InfoMessage`, сохраняется в переменную `info`.
- Для объекта `InfoMessage`, сохранённого в переменной `info`, вызвается метод,
который вернет строку сообщения с данными о тренировке. Эта строка передаётся в функцию `print()`.
